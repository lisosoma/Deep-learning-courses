...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for j in range(ceil(m / batch)):
        # get batch
        X_batch = X[j*batch:min((j+1)*batch, m)]
        I_batch = I[j*batch:min((j+1)*batch, m)]

        # SGD
        Z = np.exp(theta.T @ X_batch.T)
        Znorm = (Z / Z.sum(axis=0)).T
        gradient = ((Znorm - I_batch).T @ X_batch).T
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for j in range(ceil(m / batch)):
        ...
        gradient1 = (X_batch.T @ G1) / batch
        gradient2 = (Z1.T @ G2) / batch
        W1 -= lr * gradient1
        W2 -= lr * gradient2
